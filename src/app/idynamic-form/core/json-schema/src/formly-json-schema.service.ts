...
import { FormlyFieldConfig } from "../../src/lib/core";
...
function isConst(schema: JSONSchema7) {
  return (
    schema.hasOwnProperty("const") || (schema.enum && schema.enum.length === 1)
  );
}
      ...
      case "object": {
        ...
        Object.keys(schema.properties || {}).forEach((key) => {
          const f = this._toFieldConfig(
            <JSONSchema7>schema.properties[key],
            options
          );
          ...
          if (
            f.templateOptions &&
            !f.templateOptions.required &&
            propDeps[key]
          ) {
            f.expressionProperties = {
              "templateOptions.required": (m) =>
                m && propDeps[key].some((k) => !isEmpty(m[k])),
            };
          }
            ...
            if (
              ...
              oneOfSchema.forEach((oneOfSchemaItem) => {
                ...
                field.fieldGroup.push({
                  ...
                  hideExpression: (m) =>
                    !m || getConstValue(constSchema) !== m[key],
                });
              });
            } else {
              field.fieldGroup.push({
        ...
        if (schema.oneOf) {
          field.fieldGroup.push(
            this.resolveMultiSchema("oneOf", <JSONSchema7[]>schema.oneOf, {
              ...options,
            ...
            hideExpression: (m, fs, f) => {
              ...
              if (control.value === -1) {
                ...
                const formattedValue =
                  mode === "anyOf" ? normalizedValue : normalizedValue[0];
