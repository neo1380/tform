...
import { DynamicExtension } from "../../models";
import {
  unregisterControl,
  registerControl,
  updateValidity,
} from "../field-form/utils";
import { FormArray } from "@angular/forms";

/** @experimental */
export class FieldExpressionExtension implements DynamicExtension {
  ...
  postPopulate(field: DynamicFieldConfigCache) {
    ...
    if (!field.options.checkExpressions) {
      ...
      field.options._checkField = (f, ignoreCache) => {
        console.warn(
          `Dynamic: 'options._checkField' is deprecated since v6.0, use 'options.checkExpressions' instead.`
        );
        field.options.checkExpressions(f, ignoreCache);
      };
    }
    ...
    return (ignoreCache?: boolean) => {
      try {
        const exprValue = evalExpression(
          parentExpression
            ? (...args) => parentExpression(field) || expr(...args)
            : expr,
          { field },
        ...
        error.message = `[Dynamic Error] [Expression "${path}"] ${error.message}`;
        throw error;
      }
    };
  }
  ...
  private changeDisabledState(field: DynamicFieldConfig, value: boolean) {
    if (field.fieldGroup) {
      field.fieldGroup
        .filter(
          (f) =>
  ...
  private changeHideState(
    ...
    if (field.options.fieldChanges) {
      field.options.fieldChanges.next(<DynamicValueChangeEvent>{
        field,
        type: "hidden",
        value: hide,
      });
  ...
  private evalExpr(field: DynamicFieldConfigCache, prop: string, value: any) {
    try {
      let target = field;
      ...
      error.message = `[Dynamic Error] [Expression "${prop}"] ${error.message}`;
      throw error;
    }
