    ...
    onInit: (c) =>
      c.addConfig({
        validators: [
          ...
          {
            name: "asyncRequired",
            validation: (ctrl) => of(Validators.required(ctrl)),
          },
          {
...
describe("FieldValidationExtension: initialise field validators", () => {
  describe("built-in validations", () => {
    ...
    it("pattern", () => {
      ...
      validate(field, "ddd", {
        pattern: { requiredPattern: "^[0-9]{5}$", actualValue: "ddd" },
      });
    });
  ...
  describe("asyncValidators", () => {
    it(`using Promise`, () => {
      const field = buildField({
        asyncValidators: {
          custom: (control: FormControl) =>
            new Promise((resolve) => resolve(control.value !== "test")),
        },
      });
