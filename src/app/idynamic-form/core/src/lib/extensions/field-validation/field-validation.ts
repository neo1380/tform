...
/** @experimental */
export class FieldValidationExtension implements DynamicExtension {
  ...
  private initFieldValidation(
    ...
    if (
      type === "validators" &&
      !(field.hasOwnProperty("fieldGroup") && !field.key)
    ) {
  ...
  private wrapNgValidatorFn(
    ...
    return (control: AbstractControl) => {
      const errors: any = validatorOption.validation(
        control,
        field,
        validatorOption.options
      );
      if (isPromise(errors)) {
        return errors.then((v) =>
