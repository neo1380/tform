...
describe("DynamicUtils service", () => {
  ...
  describe("assignFieldValue", () => {
    it("should assign field value through the parent", () => {
      const parent: DynamicFieldConfig = { model: {} };
  ...
  describe("getFieldValue", () => {
    it("should properly get value", () => {
      ...
      expect(getFieldValue({ parent: { model }, key: "path.to.save" })).toBe(
        undefined
      );
      expect(getFieldValue({ parent: { model }, key: "value" })).toBe(2);
      expect(getFieldValue({ parent: { model }, key: "looks.nested" })).toBe(
        undefined
      );
      expect(getFieldValue({ parent: { model }, key: "nested.value" })).toBe(
        "bar"
      );
    });
  });
  ...
  describe("getKeyPath", () => {
    ...
    it("should attach the key path to the field config", () => {
      ...
      expect(fieldConfig["_keyPath"].path).toEqual([
        "property1",
        "property2",
        "4",
        "property3",
    ...
    it("should refresh formlyKeyPath on key updated", () => {
      const fieldConfig = { key: "property1.property2[4].property3" };
      expect(getKeyPath(fieldConfig)).toEqual([
        "property1",
        "property2",
        "4",
        "property3",
...
describe("observe", () => {
  ...
  it("should not change the enumerable descriptor", () => {
    ...
    observe(field, ["foo"], () => {});
    expect(Object.getOwnPropertyDescriptor(field, "foo").enumerable).toEqual(
      true
    );

    defineHiddenProp(field, "bar", true);
    observe(field, ["bar"], () => {});
    expect(Object.getOwnPropertyDescriptor(field, "bar").enumerable).toEqual(
      false
    );
  });
});
